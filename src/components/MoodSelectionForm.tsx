...
export function MoodSelectionForm({ previousMoodKey }: MoodSelectionFormProps) {
  ...
  return (
    <form action={startMoodSession} className="mt-7 space-y-6">
        ...
        {MOODS.map((mood) => {
          ...
          return (
            <button
              ...
              className={`rounded-[1.6rem] border p-4 text-left shadow-sm transition hover:-translate-y-0.5 hover:shadow-md ${isSelected ? "border-fuchsia-300 bg-gradient-to-br from-white to-fuchsia-50 ring-4 ring-fuchsia-100" : "border-orange-100 bg-white hover:border-pink-200"}`}
      ...
      <MoodSubmitArea selectedLabel={selectedMood ? `${selectedMood.icon} ${selectedMood.label}` : null} disabled={!selectedMoodKey} />
    </form>
  );
}

function MoodSubmitArea({ selectedLabel, disabled }: { selectedLabel: string | null; disabled: boolean }) {
  const { pending } = useFormStatus();

  return (
    <div className="rounded-[1.7rem] border border-white/70 bg-white/80 p-4 shadow-sm backdrop-blur">
      <p className="text-sm font-bold text-stone-700">{selectedLabel ? `選択中: ${selectedLabel}` : "気分アイコンを1つ選択してください"}</p>
      <p className="mt-1 text-xs leading-5 text-stone-500">更新ボタンを押すと10分セッションを開始し、ホームへ移動します。セッション中は変更できません。</p>
      <SubmitButton
