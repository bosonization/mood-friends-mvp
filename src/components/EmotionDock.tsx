...
export function EmotionDock() {
  ...
  return (
    <nav className="sticky bottom-4 z-40 mt-6 sm:hidden" aria-label="メインナビゲーション">
      <div className="mx-auto flex w-fit items-center gap-3 rounded-[2rem] border border-white/70 bg-white/72 px-4 py-3 shadow-2xl shadow-pink-100/70 backdrop-blur-2xl">
        {items.map((item) => {
            ...
            >
              <span
                className={`grid h-12 w-12 place-items-center rounded-full border text-xl transition duration-300 group-hover:-translate-y-1 ${
                  active
                    ? "border-white bg-gradient-to-br from-orange-400 via-pink-500 to-violet-600 text-white shadow-lg shadow-pink-200 ring-4 ring-pink-100"
                    : "border-white/80 bg-white/80 text-stone-700 shadow-sm shadow-stone-200/70"
                }`}
              >
                {item.icon}
              </span>
              <span className={active ? "text-pink-600" : "text-stone-500"}>{item.label}</span>
            </TransitionLink>
          );
        })}
      </div>
