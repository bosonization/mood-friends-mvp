...
const STORAGE_KEY = "emoodition:first-link-celebrated";
...
export function FirstLinkCelebration({ friendCount, level }: FirstLinkCelebrationProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 grid place-items-center bg-stone-950/35 px-5 backdrop-blur-sm">
      <div className="relative max-w-md overflow-hidden rounded-[2rem] border border-white/70 bg-white p-6 text-center shadow-2xl shadow-pink-200">
        ...
        <div className="relative">
          <div className="mx-auto grid h-24 w-24 place-items-center rounded-full bg-gradient-to-r from-orange-400 via-pink-500 to-violet-600 text-5xl text-white shadow-xl shadow-pink-100">✨</div>
          <p className="mt-5 text-sm font-black uppercase tracking-[0.22em] text-pink-600">First Link!</p>
          <h2 className="mt-2 text-3xl font-black">最初の友達とつながりました</h2>
          <p className="mt-3 leading-7 text-stone-600">
            ここからeMooditionが一気に楽しくなります。友達のノリを見て、声をかけるきっかけにしましょう。
          </p>
          ...
          <button type="button" onClick={close} className="mt-5 w-full rounded-2xl bg-gradient-to-r from-orange-500 via-pink-500 to-violet-600 px-5 py-3 font-black text-white shadow-lg shadow-pink-100">はじめる</button>
