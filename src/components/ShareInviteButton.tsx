...
export function ShareInviteButton({ memberCode, handleName, variant = "compact" }: ShareInviteButtonProps) {
    ...
    return (
      <div className="rounded-[1.7rem] border border-white/70 bg-white/75 p-4 shadow-sm backdrop-blur-xl">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            <p className="mt-2 text-sm leading-6 text-stone-600">
              URLとあなたの会員コードを一緒に共有します。LINE、SMS、Instagram DMなどはスマホの共有シートから送れます。
            </p>
          </div>
        ...
        <div className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-4">
          <button type="button" onClick={nativeShare} className="rounded-2xl bg-gradient-to-r from-orange-500 via-pink-500 to-violet-600 px-4 py-3 text-sm font-black text-white shadow-lg shadow-pink-100">
            共有
          </button>
          <a href={lineUrl} target="_blank" rel="noreferrer" className="rounded-2xl bg-[#06C755] px-4 py-3 text-center text-sm font-black text-white shadow-sm">
            LINE
          </a>
          <a href={smsUrl} className="rounded-2xl bg-gradient-to-r from-sky-500 to-cyan-500 px-4 py-3 text-center text-sm font-black text-white shadow-lg shadow-sky-100">
            SMS
          </a>
          <button type="button" onClick={copyInvite} className="rounded-2xl border border-stone-200 bg-white px-4 py-3 text-sm font-black text-stone-800 shadow-sm">
            {copied ? "コピー済み" : "コピー"}
          </button>
        </div>
        <p className="mt-3 text-xs leading-5 text-stone-500">
          InstagramはWebから本文を直接入れにくいため、共有ボタンまたはコピー後にDM/ストーリーへ貼り付けてください。
        </p>
      </div>
    );
  }
  ...
  return (
    <button
      type="button"
      onClick={nativeShare}
      className="inline-flex items-center gap-2 rounded-full border border-white/70 bg-white/80 px-3 py-2 text-sm font-black text-stone-800 shadow-sm hover:bg-white"
      aria-label="招待を共有"
    >
      <span>↗</span>
      <span>共有</span>
