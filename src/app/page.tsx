    ...
    <main className="min-h-screen overflow-hidden px-6 py-7">
      ...
      <section className="mx-auto grid max-w-6xl items-center gap-10 py-16 lg:grid-cols-[1.08fr_0.92fr] lg:py-24">
        <div>
          ...
          <h1 className="max-w-3xl text-5xl font-black leading-[1.05] tracking-tight text-stone-950 sm:text-7xl">暇とは言えない日にも、<span className="block bg-gradient-to-r from-orange-500 via-pink-500 to-violet-600 bg-clip-text text-transparent">気分は置ける。</span></h1>
        ...
        <div className="relative">
          ...
          <div className="relative rounded-[2.3rem] border border-white/70 bg-white/75 p-5 shadow-2xl shadow-orange-100 backdrop-blur-xl">
            <div className="rounded-[1.8rem] bg-stone-950 p-5 text-white shadow-xl">
              ...
              <div className="grid grid-cols-2 gap-3">{[["🍚", "食事", "12分前"], ["🎮", "ゲーム", "28分前"], ["💬", "会話", "42分前"], ["❤️", "ウキウキ", "3時間前"]].map(([icon, mood, time]) => <div key={mood} className="rounded-3xl border border-white/10 bg-white/10 p-4"><p className="text-4xl">{icon}</p><p className="mt-3 font-black">{mood}</p><p className="mt-1 text-xs text-stone-400">{time}</p></div>)}</div>
            </div>
          </div>
        </div>
      </section>

      <section id="features" className="mx-auto grid max-w-6xl gap-4 pb-8 md:grid-cols-3">{[["10分で固定", "選んだ気分はセッション中変更不可。更新頻発を防ぎます。"], ["友達だけ", "会員コードと相互承認で、既につながっている人だけに表示。"], ["チャットなし", "アプリ内で連絡先やID交換をさせない設計です。"]].map(([title, body]) => <div key={title} className="rounded-[1.7rem] border border-white/70 bg-white/70 p-6 shadow-sm backdrop-blur"><h2 className="text-lg font-black">{title}</h2><p className="mt-2 text-sm leading-6 text-stone-600">{body}</p></div>)}</section>
      <div className="mx-auto max-w-6xl"><AdSlot /></div>
