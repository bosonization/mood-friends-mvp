...
export async function signIn(formData: FormData) {
  ...
  if (error) redirect(`/login?message=${encodeURIComponent("ログインに失敗しました。メールアドレスとパスワードを確認してください。")}`);
...
export async function signUp(formData: FormData) {
  ...
  if (error) redirect(`/login?message=${encodeURIComponent("会員登録に失敗しました。別のメールアドレスを試してください。")}`);
  redirect("/onboarding");
}
