    ...
    <main className="grid min-h-screen items-center px-6 py-10">
      <section className="mx-auto grid max-w-5xl gap-8 lg:grid-cols-[0.92fr_1.08fr]">
        <div className="flex flex-col justify-center">
          <TransitionLink href="/" className="mb-8 inline-flex w-fit rounded-full border border-white/70 bg-white/70 px-4 py-2 text-sm font-bold backdrop-blur">
            ← トップへ
          </TransitionLink>
        ...
        <section className="rounded-[2rem] border border-white/70 bg-white/80 p-5 shadow-xl shadow-orange-100 backdrop-blur-xl">
          ...
          <div className="mt-4 grid gap-4 md:grid-cols-2">
            <form action={signIn} className="rounded-3xl border border-stone-100 bg-white p-5 shadow-sm">
              ...
              <SubmitButton pendingText="ログイン中..." className="mt-5 w-full rounded-2xl bg-gradient-to-r from-orange-500 via-pink-500 to-violet-600 px-5 py-3 font-black text-white shadow-lg shadow-pink-100">
                ログイン
              </SubmitButton>
            </form>
            ...
            <form action={signUp} className="rounded-3xl border border-orange-100 bg-orange-50/70 p-5 shadow-sm">
              ...
              <label className="mt-5 block text-sm font-bold">
                メールアドレス
                <input
                  className="mt-2 w-full rounded-2xl border border-orange-200 px-4 py-3 outline-none focus:border-pink-400"
                  name="email"
                  type="email"
                  required
                  autoComplete="email"
              ...
              <p className="mt-3 text-xs leading-5 text-stone-500">
                入力間違い防止のため、パスワードは2回入力してください。
              </p>

              <SubmitButton pendingText="登録中..." className="mt-5 w-full rounded-2xl bg-stone-950 px-5 py-3 font-black text-white hover:bg-stone-800">
                登録する
              </SubmitButton>
            </form>
          </div>
