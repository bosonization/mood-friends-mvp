...
export default async function FriendsPage({ searchParams }: FriendsPageProps) {
  ...
  const { data: relatedProfiles } = relatedIds.length
    ? await supabase.from("profiles").select("*").in("id", relatedIds).returns<Profile[]>()
    : { data: [] as Profile[] };
      ...
      <div className="grid gap-6 lg:grid-cols-[0.86fr_1.14fr]">
        <section className="rounded-[2rem] border border-white/70 bg-white/85 p-5 shadow-sm backdrop-blur-xl">
          ...
          <form action={requestFriend} className="mt-5 space-y-3">
            <label className="block text-sm font-bold">相手の会員コード
              <input className="mt-2 w-full rounded-2xl border border-stone-200 px-4 py-3 font-mono tracking-widest outline-none focus:border-pink-400" name="memberCode" inputMode="numeric" maxLength={10} placeholder="1234567890" required />
            </label>
            <button className="w-full rounded-2xl bg-gradient-to-r from-orange-500 via-pink-500 to-violet-600 px-5 py-3 font-black text-white shadow-lg shadow-pink-100">申請する</button>
          </form>
        ...
        <section className="space-y-5">
          <FriendshipSection title="承認待ち" empty="届いている申請はありません。">
            {receivedPending.map((friendship) => {
              const other = profileById.get(friendship.requester_id);
              if (!other) return null;
              return <FriendshipCard key={friendship.id} profile={other}><div className="flex gap-2"><form action={acceptFriend}><input type="hidden" name="friendshipId" value={friendship.id} /><button className="rounded-full bg-stone-950 px-4 py-2 text-sm font-bold text-white">承認</button></form><form action={rejectFriend}><input type="hidden" name="friendshipId" value={friendship.id} /><button className="rounded-full border border-stone-200 px-4 py-2 text-sm">拒否</button></form></div></FriendshipCard>;
            })}
          </FriendshipSection>

          <FriendshipSection title="申請中" empty="送信中の申請はありません。">
            {sentPending.map((friendship) => {
              const other = profileById.get(friendship.addressee_id);
              if (!other) return null;
              return <FriendshipCard key={friendship.id} profile={other}><form action={deleteFriend}><input type="hidden" name="friendshipId" value={friendship.id} /><button className="rounded-full border border-stone-200 px-4 py-2 text-sm">取消</button></form></FriendshipCard>;
            })}
          </FriendshipSection>
          ...
          <FriendshipSection title="友達" empty="まだ友達はいません。">
            {accepted.map((friendship) => {
              ...
              if (!other) return null;
              return <FriendshipCard key={friendship.id} profile={other}><form action={deleteFriend}><input type="hidden" name="friendshipId" value={friendship.id} /><button className="rounded-full border border-red-200 px-4 py-2 text-sm text-red-700">削除</button></form></FriendshipCard>;
            })}
          </FriendshipSection>
        </section>
      </div>
