...
export default async function FriendsPage({ searchParams }: FriendsPageProps) {
  ...
  const receivedPending = (friendships ?? []).filter(
    (item) => item.status === "pending" && item.addressee_id === user.id
  );
  const sentPending = (friendships ?? []).filter(
    ...
    <AppShell>
      <div className="grid gap-6 lg:grid-cols-[0.86fr_1.14fr]">
        <section className="rounded-[2rem] border border-orange-100 bg-white/85 p-5 shadow-sm">
          <p className="text-sm font-medium text-orange-700">Friend Code</p>
          <h1 className="text-2xl font-black">会員コードで友達申請</h1>
          <p className="mt-2 text-sm leading-6 text-stone-600">
            10桁の会員コードを知っている相手にだけ申請できます。公開検索はありません。
          </p>
          ...
          <form action={requestFriend} className="mt-5 space-y-3">
            <label className="block text-sm font-bold">
              相手の会員コード
              <input
                className="mt-2 w-full rounded-2xl border border-stone-200 px-4 py-3 font-mono tracking-widest outline-none focus:border-orange-400"
                name="memberCode"
                inputMode="numeric"
                maxLength={10}
            ...
            <button className="w-full rounded-2xl bg-orange-500 px-5 py-3 font-bold text-white hover:bg-orange-600">
              申請する
            </button>
          </form>
        ...
        <section className="space-y-5">
          <FriendshipSection title="承認待ち" empty="届いている申請はありません。">
            {receivedPending.map((friendship) => {
              ...
              return (
                <FriendshipCard key={friendship.id} profile={other}>
                  <div className="flex gap-2">
                    <form action={acceptFriend}>
                      <input type="hidden" name="friendshipId" value={friendship.id} />
                      <button className="rounded-full bg-stone-950 px-4 py-2 text-sm font-bold text-white">
                        承認
                      </button>
                    </form>
                    <form action={rejectFriend}>
                      <input type="hidden" name="friendshipId" value={friendship.id} />
                      <button className="rounded-full border border-stone-200 px-4 py-2 text-sm">
                        拒否
                      </button>
                    </form>
                  </div>
            ...
            {accepted.map((friendship) => {
              ...
              return (
                <FriendshipCard key={friendship.id} profile={other}>
                  <form action={deleteFriend}>
                    <input type="hidden" name="friendshipId" value={friendship.id} />
                    <button className="rounded-full border border-red-200 px-4 py-2 text-sm text-red-700">
                      削除
                    </button>
                  </form>
                </FriendshipCard>
    ...
    <div className="rounded-[2rem] border border-orange-100 bg-white/85 p-5 shadow-sm">
      ...
      <div className="mt-4 space-y-3">
        {Array.isArray(items) && items.length === 0 ? (
          <p className="rounded-3xl border border-dashed border-orange-200 p-5 text-sm text-stone-600">
            {empty}
          </p>
        ) : (
          items
...
function FriendshipCard({
  ...
  return (
    <article className="flex items-center gap-3 rounded-3xl border border-stone-100 bg-white p-4">
